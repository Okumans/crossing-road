...
std::shared_ptr<Texture> IBLGenerator::generateIrradianceMap(const Texture& skybox, const Skybox& cubeMesh, Shader& irradianceShader) {
    ...
    // We still need to bind the FBO for rendering, as glDrawArrays is not DSA for the framebuffer target itself
    // and glViewport affects the global state.
    glViewport(0, 0, 32, 32); 
    glBindFramebuffer(GL_FRAMEBUFFER, captureFBO);
    for (unsigned int i = 0; i < 6; ++i)
    {
        irradianceShader.setMat4("u_View", captureViews[i]);
        glNamedFramebufferTextureLayer(captureFBO, GL_COLOR_ATTACHMENT0, irradianceMap, 0, i);
    ...
    return std::make_shared<Texture>(irradianceMap, TextureType::IRRADIANCE_MAP, true);
}
