...
void GrassRow::drawSidePanel(const RenderContext &ctx, float z,
  ...
  model_top =
      glm::scale(model_top, glm::vec3(WIDTH, draw_top_edge_height, m_depth));

  ctx.shader.setMat4("u_Model", model_top);
  ctx.shader.setVec2("u_UVOffset", m_uvOffset);
  // Scale UV to match top surface texture tiling roughly
  ctx.shader.setVec2("u_UVScale",
                     glm::vec2(WIDTH / getUVScaleFactor(),
                               draw_top_edge_height / getUVScaleFactor()));

  s_sideMesh->draw(ctx, m_material);
  ...
  if (total_height > top_edge_height) {
    ...
    model_side =
        glm::scale(model_side, glm::vec3(WIDTH, main_side_height, m_depth));

    ctx.shader.setMat4("u_Model", model_side);
    ctx.shader.setVec2("u_UVOffset", m_uvOffset);
    ctx.shader.setVec2(
        "u_UVScale",
        glm::vec2(WIDTH / getUVScaleFactor() * side_detail_multiplier,
